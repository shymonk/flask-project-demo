...
- 🎯 Entry points to execute your program using `python -m <flask_project_demo>` or `$ flask_project_demo` with basic CLI argument parsing.
...
---
# flask_project_demo Flask Application

Awesome flask_project_demo created by shymonk
...
```bash
git clone https://github.com/shymonk/flask-project-demo flask_project_demo
cd flask_project_demo
make install
```
...
```bash
pip install flask_project_demo
```
...
```bash
flask_project_demo create-db   # run once
flask_project_demo populate-db  # run once (optional)
flask_project_demo add-user -u admin -p 1234  # ads a user
flask_project_demo run
```
