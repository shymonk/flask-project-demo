...
original_author="shymonk"
original_name="flask_project_demo"
original_urlname="flask-project-demo"
original_description="Awesome flask_project_demo created by shymonk" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/flask_project_demo "${name}"
...
then
    ...
    echo "then run 'flask_project_demo' or 'python -m flask_project_demo'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'flask_project_demo' or 'python -m flask_project_demo' or 'poetry run flask_project_demo'"
fi
